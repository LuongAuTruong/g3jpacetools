...
class G3Encoding(codecs.Codec):
    def __init__(self) -> None:
        super().__init__()
        self.encoding_map = MappingProxyType({   
            0x0 : '　',
        ...
        self.reverse_encoding_map = MappingProxyType(
            {char: codepoint for codepoint, char in self.encoding_map.items()}
        )

    def encode(self, input : str, errors='strict') -> tuple[bytes, int]:
        output = bytearray()
        for char in input:
            if char in self.reverse_encoding_map:
                output.append(self.reverse_encoding_map[char])
            else:
                raise UnicodeEncodeError(
                    'G3EncodingJP', input, 0, len(input), 'unsupported character'
                )
    ...
    def decode(self, input: bytes, errors: str = 'strict') -> tuple[str, int]:
        ...
        for char in input:
            if char in self.encoding_map:
                output.append(self.encoding_map[char])
            else:
...
def search_g3_encoding(encoding_name):   
    return codecs.CodecInfo(
        encode=G3Encoding().encode, 
        decode=G3Encoding().decode, 
        name='G3EncodingJP',
    )


codecs.register(search_g3_encoding)